...
import { CellTowerIcon } from '@bmi/mui-tonic-icons';
import type { IconSize, IconVariant, IconContainedShape } from '@bmi/mui-tonic-icons';

const meta: Meta<typeof CellTowerIcon> = {
  title: 'Icons/CellTowerIcon',
  component: CellTowerIcon,
  parameters: {
    layout: 'centered',
  },
  argTypes: {
    ...
    <Stack direction="row" spacing={4} alignItems="center">
      {(['outlined', 'filled', 'contained'] as const).map((v) => (
        <Stack key={v} alignItems="center" spacing={1}>
          <CellTowerIcon {...args} variant={v} size="medium" />
      ...
      {(['primary', 'secondary', 'error', 'warning', 'info', 'success', 'action', 'disabled'] as const).map(
        (color) => (
          <Stack key={color} alignItems="center" spacing={1}>
            <CellTowerIcon {...args} size="medium" color={color} />
            <Typography variant="caption" color="text.secondary">
              {color}
            </Typography>
          </Stack>
    ...
    <Stack direction="row" spacing={4} alignItems="center">
      {(['circle', 'square', 'rounded'] as const).map((shape) => (
        <Stack key={shape} alignItems="center" spacing={1}>
          <CellTowerIcon {...args} variant="contained" containedShape={shape} size="medium" />
          <Typography variant="caption" color="text.secondary">
            {shape}
          </Typography>
        </Stack>
