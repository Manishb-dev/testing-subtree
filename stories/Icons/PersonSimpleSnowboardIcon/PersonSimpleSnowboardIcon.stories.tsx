...
import { PersonSimpleSnowboardIcon } from '@bmi/tonic-icons-web';
import type { IconSize, IconVariant, IconContainedShape } from '@bmi/tonic-icons-web';

const meta: Meta<typeof PersonSimpleSnowboardIcon> = {
  title: 'Icons/PersonSimpleSnowboardIcon',
  component: PersonSimpleSnowboardIcon,
  parameters: {
    layout: 'centered',
  },
  argTypes: {
    ...
    <Stack direction="row" spacing={4} alignItems="center">
      {(['outlined', 'filled', 'contained'] as const).map((v) => (
        <Stack key={v} alignItems="center" spacing={1}>
          <PersonSimpleSnowboardIcon {...args} variant={v} size="medium" />
    ...
    <Stack direction="row" spacing={3} alignItems="flex-end">
      {(['small', 'medium', 'large'] as const).map((s) => (
        <Stack key={s} alignItems="center" spacing={1}>
          <PersonSimpleSnowboardIcon {...args} size={s} />
          <Typography variant="caption" color="text.secondary">
            {s}
          </Typography>
        </Stack>
      ...
      {(['primary', 'secondary', 'error', 'warning', 'info', 'success', 'action', 'disabled'] as const).map(
        (color) => (
          <Stack key={color} alignItems="center" spacing={1}>
            <PersonSimpleSnowboardIcon {...args} size="medium" color={color} />
            <Typography variant="caption" color="text.secondary">
              {color}
            </Typography>
          </Stack>
    ...
    <Stack direction="row" spacing={4} alignItems="center">
      {(['circle', 'square', 'rounded'] as const).map((shape) => (
        <Stack key={shape} alignItems="center" spacing={1}>
          <PersonSimpleSnowboardIcon {...args} variant="contained" containedShape={shape} size="medium" />
          <Typography variant="caption" color="text.secondary">
            {shape}
          </Typography>
        </Stack>
